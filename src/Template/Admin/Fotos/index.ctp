...
<!--              </div>-->
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover">
            <thead>
              <tr>
                  <th scope="col"><?= $this->Paginator->sort('albun_id', ['label' => 'Álbum']) ?></th>
            ...
            <tbody>
              <?php foreach ($fotos as $foto): ?>
                <tr>
                  <td><?= $foto->has('albun') ? $this->Html->link($foto->albun->titulo, ['controller' => 'Albuns', 'action' => 'view', $foto->albun->titulo]) : '' ?></td>
                  <td><?= h($foto->titulo) ?></td>
                    <td><img style="width: 80px; border-radius: 10px" src="/files/Fotos/imagem/<?= $foto->imagem ?>"></td>
                  <td class="actions text-right">
                      <?= $this->Html->link(__('Edit'), ['action' => 'edit', $foto->id], ['class'=>'btn btn-warning']) ?>
